...
SRC_DIR="/media/simon/PortableSSD/simon-media/Mediathek/Musikvideos"
DEST_DIR="$(pwd)/mp3_library"
PLAYLIST_PATH="$(pwd)/playlist.m3u"
...
convert_file() {
    ...
    if [[ -f "$dest" ]]; then
        printf 'Überspringe vorhandene Datei: %s\n' "$dest"
        return 0
    fi
    printf 'Konvertiere: %s -> %s\n' "$display" "$dest"
    if ffmpeg -hide_banner -nostdin -loglevel error -i "$src" -vn -ac 2 -ar 44100 -c:a libmp3lame -b:a 256k "$dest"; then
        return 0
    else
